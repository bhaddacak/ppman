...
At the level of file as shown by the icon, the document can be opened with a context menu provided. Behind the name of a document, its reference according to the criteria of the corpus is shown in parentheses. For example, in the picture \emph{d1} (or D1) is used for Sīlakkhandhavagga of Dīghanikāya in the CSTR collection.
...
\newpage
\boxnote{In \textsc{Pāli\,Platform\,3}, the window of TOC Tree has simpler functionality comparing to the former versions. It can just browse a document in its structure and open it.\\
\hspace{5mm}The function of bookmarking is now removed, together with the Tokenizer and its related tasks.\footnote{Now it has no way to bookmark a document. Using Document Finder is advisable. The obsolete Tokenizer is now removed from the program because its functions are mostly replaced by the Lucene module.}
}
