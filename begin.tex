...
Originally, I had an idea to make a program that can search words in the Pāli canon effectively, at least more effective than the programs we had at the time. That was around 3 decades ago, when I was interested in Buddhism after I finished my undergrad engineering program. At the time, it was too difficult to do with limited technology and data available, and I had many more pressing things to do. So, the idea just got lingered in my mind since.

After I found that we have a digital version of the whole Pāli collection, distributed by Vipassana Research Institute (VRI) via \url{tipitaka.org}, the possibility of the project began to light up. But I still had more important things to do. Once I quit my job, I had an opportunity to continue my studies. Then I finished a master program in Software Engineering.\footnote{Meanwhile, I also studied Buddhism and Philosophy at another university, and finished it in a semester afterward.} The subject of Information Retrieval (IR) was the main concern of that study.
...
After I learned that the structure of the Pāli canon provided by VRI is really frustrating to work with, I spent several months to restructure the collection\footnote{This can be seen at \url{bhaddacak.github.io/cstpage}. All materials are hosted in \url{github.com/bhaddacak/cst-kit}. In \textsc{Pāli\,Platform\,3}, this collection is abbreviated as \emph{CSTR} (Chaṭṭha Saṅgāyana Tipiṭaka Restructured).}. Apart from that, I also found digital versions of several Pāli corpora, including the Pali Text Society edition, Siam Rath edition, SuttaCentral edition, and the recent revival of Buddha Jayanthi edition. So, I incorporated all these into \textsc{Pāli\,Platform\,3}, together with my own collection of Pāli grammar books.

Another player that makes Pāli studies exciting recently is the \emph{Digital Pāḷi Dictionary} (DPD), a massive Pāli dictionary. When the dictionary has made data available in a database format, it is now ready to be utilized. This is another factor leading to development of \textsc{Pāli\,Platform\,3}.

To make all features mentioned above available, many things have to be changed and added (some have to be removed as well). And to make it easier to maintain, now I fully adopt the \emph{Java Platform Module System} (JPMS) in the development.\footnote{This technical thing means nothing to the users. But for Java developers, it is essential to know and master the system.} \textsc{Pāli\,Platform\,3} has its first launch around the end of 2024.

In 2025 after I finished a translation of Kaccāyana grammar\footnote{\emph{Grammatical Suttas of Kaccāyana}, see \url{bhaddacak.github.io/ksut}.}, I found that the knowledge of Sanskrit is really useful when we dig deeper in Pāli grammar. Since then I have improved many things in the program and added supports for Sanskrit learning. These include all major Sanskrit dictionaries, a Sanskrit text collection, and some grammatical tools. Finally, we have seen the launch of \textsc{Pāli\,Platform\,4} at the start of 2026. Now the program has everything you need to master Pāli (and Sanskrit).
...
The program can do many things related to Pāli studies. Before we go into details of these in our course, let us see the big picture. I group the main functionality into six:
...
\item CST Devanāgarī Base (Tipitaka.org XML)\footnote{This is the very source of the Chaṭṭha Saṅgāyana Tipiṭaka as maintained by VRI's Tipitaka-XML project (\url{github.com/vipassanatech/tipitaka-xml}). All digital versions found in the Internet are based on this collection.}
\item CST4 data (customized)\footnote{Formerly, This is the data bundled with the program CST4.1, but now it is converted to Roman from the Devanāgarī base above. It is also undergone some editing by the author.}
...
\item The Pali Text Society Tipiṭaka (PTST)\footnote{A digital version of this corpus is hosted at GRETIL (\url{gretil.sub.uni-goettingen.de}).}
...
\item Traditional Pāli Grammar Books\footnote{This collection is prepared by the author from various sources. Some Kaccāyana related books have been added recently.}
\end{compactitem}
...
A recent addition is the support of Margaret Cone's dictionary, which has to be installed manually by the user.
...
\item Comprehensive Pāli root finder
\item Grammatical sutta finder
\end{compactitem}

\paragraph{(4) Powerful search tools.} Searching is the primary goal of the program. So, we can see various approaches for finding things here. At basic level, texts in the collections can be searched by \emph{Document Finder} (in menu Reader), either by document information, by abbreviations, or by textual content (brute full text search). For a more advanced search, \emph{Lucene Finder} can be used with its full capacity. The users can build Lucene's indexes with options adjusted as needed. With these indexes, the users can also see all terms and their frequency by using \emph{Term Lister}.

\paragraph{(5) Sanskrit tools.} Apart from Pāli dictionaries, we can also search in all major Sanskrit dictionaries (downloaded on demand), including the followings:

\begin{compactitem}
\item M. Monier-Williams' Sanskrit-English Dictionary (1899)
\item V.\,S. Apte's Practical Sanskrit-English Dictionary (revised, 1957)
\item Jīvānanda Vidyāsāgara Bhaṭṭācāryya's Śabda-sāgara \\(1900)
\item Arthur A. Macdonell's Sanskrit-English Dictionary (1893)
\item Franklin Edgerton's Buddhist Hybrid Sanskrit Dictionary (1953)
\item M. Monier-Williams's English-Sanskrit Dictionary (1851)
\item V.\,S. Apte's Student's English-Sanskrit Dictionary (1964)
\item Anundoram Borooah's English-Sanskrit Dictionary (1877-1881)
\end{compactitem}

For Sanskrit text, we can access the whole collection of Sanskrit documents hosted in GRETIL. Several grammatical tools have been added, including: list of sandhi rules, sandhi combiner and analyzer, nominal paradigms and declension table, verbal paradigms and conjugation table, and Whitney's roots.

\paragraph{(6) Unique and helpful accessories.} The program also incorporates a number of general tools, such as \emph{Text Editor} (with several helpful Pāli and Sanskrit related functions), \emph{Sentence Reader} and \emph{Sentence Manager} (with ability to aid text reading and to add translations), \emph{Batch Script Transformer} (to convert a bunch of text files from one script to another), and some CLI tools for technical uses. In the recent development, the transliterating engine has been improved significantly with several options added. With Sanskrit support, now the methods of entering text into the program are more versatile and effective.

\section{How to run the program}

Now we come to the technical instruction. Theoretically, the program can work in all devices that are able to run \emph{Java Virtual Machine} (JVM)\footnote{JVM is the environtment in which Java executable code runs. It is a part of \emph{Java Development Kit} (JDK). For users, you do not need the whole JDK. Just the part called \emph{Java Runtime Environment} (JRE) is enough to run the program.}, in three major operating systems (OS), i.e., Windows, macOS, and GNU/Linux (including other Unix variants such as BSD-based OS). These include 32/64-bit Intel/AMD-based computers, AArch64-based devices, 32-bit ARM devices.\footnote{For 32-bit ARM, manual installation of JavaFX is required.} The requirement of resources is very low. Even basic RAM of 2 GB is fine. However, running the program on a more powerful machine certainly brings you quicker results.

\subsection{Where to get and put the program}

Now that the program's source code is opened as a \texttt{Github} repository, its releases can be found at \url{github.com/bhaddacak/paliplatform/releases}. Normally, you will find only the most recent version (and some patches, if any). So, you can download the program package from the release's assets. When multiple versions are found, it is advisable to use the most recent one. For patches, you do not need to download them because the program has its own updating system.\footnote{For manual patching, see an instruction in \url{bhaddacak.github.io/platform4}.}
...
In the release's assets and notes, you may find other resources, such as an alternative text collection (explained below) and additional dictionary files. You may download these as well.

Once you get the program's package, move it to a place where you have full write-permission, and unpack it with your unpacker. Then you can see a folder named \texttt{PaliPlatform}. The program needs no further installation. Everything created by the program will reside only there. If you need a convenient launcher/shortcut on your desktop, use {\faSix \symbol{"F135}} \emph{Create desktop launcher} in the program's menu File when it successfully runs.

\boxnote{Icon's image of the desktop shortcut\\
\hspace{5mm} \bullet\ In Windows, the executable launcher has a small icon embedded. Once the shortcut is created, this icon is used. You can change the icon's image in the shortcut's properties. The program provides a number of icon files in directory \texttt{data/pic}. These icons have multiple sizes and look better.\\
\hspace{5mm} \bullet\ In Linux, the shortcut uses the default icon file in the directory mentioned above. You can change to another file by editing the launcher's properties.\\
\hspace{5mm} \bullet\ In macOS, forget it. I do not know how to make such a shortcut.
}

\newpage
If you are not a naive user that mostly just clicks and sees (and complains sometimes), it is helpful to know the file structure of the program as shown in Figure \ref{fig:folder-structure} (What is really shown in your computer is likely different, but the content should be mostly the same).

\begin{figure}[!hbt]
\centering
\includegraphics[width=0.9\linewidth]{images/folder-structure.png}\\
\caption{Folder structure of Pāli\,Platform\,4}
\label{fig:folder-structure}
\end{figure}

Note that some files will not appear in your first run, i.e., those ending with \texttt{.properties}, some directories in \texttt{data} will not be present yet, and some directories will be created in the first run, i.e., \texttt{cache} and \texttt{output}. If you have downloaded the \texttt{winready} package, you will see the \texttt{jre} folder, otherwise you will not. If you see things similar to what is shown, you are ready to run the program now.
...
\paragraph{Method 1: Use \texttt{winready} package.} This is the easiest way, but only for 64-bit machines. After you unpack the package and go into the program's folder, just double-click \texttt{PPLauncher.exe} (the file having the program's logo, in case you do not see the extension). That is all. The only problem that fails the operation is you have downloaded a wrong package. Make sure that you see \texttt{jre} folder before launching the program. After you can make the program start, and you can create a desktop launcher by the menu provided.
...
\paragraph{Method 6: Do it yet a harder way.} This method is for Java veterans only. It can be used when a JVM and JavaFX are present properly. See the instruction in the box below.
...
\paragraph{Method 1: Install JRE + JavaFX system-wide (recommended)} When downloading Java, select installation packages with JavaFX included.\footnote{It does not matter whether you use JDK or JRE. For the former choice, you can also develop your own Java programs, but the latter is far smaller in size. For macOS, installation packages are of either \texttt{.pkg} or \texttt{.dmg} files. Be aware that older macOS cannot run newer Java. For example, my Catalina can run only Java 11, not the newer.} Once the installation is successful, you can double-click \texttt{PPLauncher.jar}.\footnote{This does not always work due to some security problem. The system just denies to run a program from an unknown source. If this occurs, use Step 3 of the Linux's method. As far as I can say, my Catalina does not have this problem.} Or open a terminal\footnote{In macOS, to open a terminal at a folder, you just simply drag the target folder icon to the Terminal icon on the Dock panel below. Another little harder way is right-clicking the folder icon. Then select Services > New Terminal at Folder.} at the program's root directory and follow Step 3 of the Linux's method. After the program works properly, you can create a desktop launcher by the menu provided. This desktop launcher works only the system has Java installed (and invoking \texttt{PPLauncher.jar} works properly).

\boxnote{One problem you may encounter when installing Java to macOS system is that you already have an existing Java without JavaFX. In this case, you have to remove the old Java first. If your java has an uninstaller, use that one. If not, you must do it manually by opening a terminal and type these commands:\\
\commandline{cd /Library/Java/JavaVirtualMachines}\\
\hspace{5mm}(You do not need to type all of this, just some letters and press Tab.)\\
\commandline{ls}\\
...
After that you will see a dialog similar to that in Figure \ref{fig:jfxinstall}. The installer will detect your environment and offer you choices to download and install. This depends on what kind of OS you use, what architecture of your machine, and what version of your Java. The versions of JavaFX available here are predefined. They may not be the latest, but they underwent certain testing.
...
\caption{Main window of \textsc{Pāli\,Platform\,4}}
...
Here are some useful links related to the program's execution. If you use JRE bundled with JavaFX, recommended versions are 11.0.26 or lower, 17.0.14 or lower, and 21.0.6 or lower. If you have to separate JRE and JavaFX, Java 11 should be used with JavaFX 19.0.10 or lower, and Java 17 or newer should be used with JavaFX 21.0.6 or lower.\footnote{JavaFX newer than January 2025 seems to have certain problem in displaying Devanāgarī and other scripts, probably caused by the WebKit component.} In 32-bit machines, using Java 11 is preferable.


Higher versions than those (after January 2025) do not properly display Devanāgarī text in HTML viewers. 
...
\paragraph{(2) BellSoft Liberica JDK.} BellSoft also provides installers of JRE with JavaFX for major platforms. This can be easy for new users who just want to install what is needed. When downloading, select JRE Full. This vendor is recommended for macOS.
...
\paragraph{(4) Gluon's JavaFX.} This is the official site of JavaFX. You rarely need this, except if you want the newest version of it (really unnecessary for us). When downloading, select SDK binary package, not \texttt{jmods}. Also see the instruction on JavaFX manual installation described above.
