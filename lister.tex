...
When I was strenuously writing my Pāli books, this tool was one of the most used. Now it has capability to handle all text collections, thanks to Lucene. In the previous versions of the program, this tool used pre-built term lists. Now it uses term lists from indexes created by Lucene Finder. This means, essentially, you have to know how to build an index from a text collection first (see Chapter \ref{chap:lucene}). Once you have an index of the collection, you can explore its term list.

\dothis{To use Term Lister, click {\faSix \symbol{"F0C9}} button on the main tool bar, or use menu Lucene > {\faSix \symbol{"F0C9}} Term Lister. Or you can use its tab in the main window by loading the tool when needed.\\
...
When you are satisfied with the listing options, then hit {\faSix \symbol{"F7D9}} Generate button and wait a minute (maybe shorter or longer depending on the index size). Once the creation is done, you will have the table's name in the top selector. When this table is selected, its content will show in the table area below (see Figure \ref{fig:lister-table}).

\boxnote{The name of a lister table created is automatically set according to its source index and options. The abbreviations are intuitive and easily recognized, but you may have to get familiar with the system.}
...
What counted as `term' here is simply a token. It is not a Pāli word in strict sense. A token, in the domain of computational Pāli, is a chunk of character string without in-between separators. So, a portion with punctuation marks will be split with those marks resulting in standalone tokens. The tokenization process is done by indexing (Lucene Finder). All tokens are also normalized into lowercase letters.\footnote{Unlike the former versions of the program, the option of keeping capitalized terms is now removed. Because I found that in CSTR collection (the former CSCD), the capitalization was somewhat incomplete or not good enough for analyses. Moreover, I am likely to prefer uncapitalized text when making a text collection.}
...
By default, the result is retrieved only 500 rows, ordered by top frequencies. This can quicken the starting time. You can set the number of maximum rows by a drop-down option in the tool bar, ranging from 50 to 1,000,000 rows. The less you select the faster you get the result.
...
If you choose the full frequency range (\textbf{>=\,1}) like me, you have to wait a little long.\footnote{It is processed under 10 seconds each step in my computer, pretty fast in fact. It is long because the result does not come up right away.} It will be faster if you choose \textbf{=\,1} instead (very long terms are supposed to appear once), because less data will be processed.

You can use a word in the result table or send it to other tools by right-clicking the row you want to open its context menu. Then four choices are available as follows (see their exact wording in Figure \ref{fig:lister-contextmenu}):

\begin{compactenum}[(1)]
\item To copy the word to the system's clipboard
\item To send the word to the main Dictionaries tab
\item To send the word to the main Document Finder tab
\item To send the word to the main Lucene Finder tab
\end{compactenum}

\dothis{To copy a term in the lister result, simply drag and drop it to a program's Text Editor opened. Also try dropping elsewhere.\\
...
The simplest way to filter the results is selecting by the starting letters. When you enter a letter, terms starting with that letter will show up (see Figure \ref{fig:lister-filter-simple}). Entering next characters will refine the filtering further.
...
Here, the wildcards, ? (any one character) and * (any number of characters including none), also have their uses, and they are quite helpful if you can imagine how to put them together. See Figure \ref{fig:lister-filter-wildcards} for example.
...
If you know how to use regular expressions, you can gain a lot of benefits from this mode of filtering. Figure \ref{fig:lister-filter-regexp} shows an example of filtering by an regular expression.\footnote{This is a hit-and-run example to find short words that use \pali{t} or \pali{ṭ} in the second syllable, doubled or not, and followed by one vowel. You may not have a chance to use this pattern, but it tells you a lot about how regular expressions work.} For more information about the syntax, see Chapter \ref{chap:regexp}.
...
\caption{Grouping by the last 2 letters}
